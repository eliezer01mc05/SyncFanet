...
Define_Module(inet::PingTestApp);
Register_Class(PingTestApp);

simsignal_t PingTestApp::rttSignal = registerSignal("rtt");
simsignal_t PingTestApp::numLostSignal = registerSignal("numLost");
simsignal_t PingTestApp::numOutOfOrderArrivalsSignal = registerSignal("numOutOfOrderArrivals");
...
void PingTestApp::parseDestAddressesPar()
{
...
void PingTestApp::handleMessage(cMessage *msg)
{
    if (!isNodeUp()) {
        if (msg->isSelfMessage())
            ...
            EV_WARN << "PingTestApp is down, dropping '" << msg->getName() << "' message\n";
            delete msg;
            return;
        }
    }
...
void PingTestApp::refreshDisplay() const
{
    char buf[40];
...
bool PingTestApp::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void PingTestApp::startSendingPingRequests()
{
...
void PingTestApp::stopSendingPingRequests()
{
    pid = -1;
    lastStart = -1;
    sendSeqNo = expectedReplySeqNo = 0;
...
void PingTestApp::scheduleNextPingRequest(simtime_t previous, bool withSleep)
{
    simtime_t next;
...
void PingTestApp::cancelNextPingRequest()
{
    cancelEvent(timer);
}

bool PingTestApp::isNodeUp()
{
    return !nodeStatus || nodeStatus->getState() == NodeStatus::UP;
}

bool PingTestApp::isEnabled()
{
